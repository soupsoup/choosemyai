-- ChooseMyAI Database Schema for Supabase
...
INSERT INTO tool_categories (tool_id, category_id) VALUES
  ...
  (1, 5), -- ChatGPT - AI Research
  (2, 2), -- Midjourney - AI Image Generation
  (3, 7); -- GitHub Copilot - AI Development
...
CREATE POLICY "Public read access to approved tools" ON tools FOR SELECT USING (is_approved = true);
CREATE POLICY "Public read access to tool categories" ON tool_categories FOR SELECT USING (true);
CREATE POLICY "Public read access to comments" ON comments FOR SELECT USING (true);
CREATE POLICY "Public read access to published blog posts" ON blog_posts FOR SELECT USING (published = true);
CREATE POLICY "Public read access to appearance settings" ON appearance_settings FOR SELECT USING (true);

-- Create policies for authenticated users
CREATE POLICY "Users can create tools" ON tools FOR INSERT WITH CHECK (auth.uid() = user_id);
...
CREATE POLICY "Users can vote on tools" ON tool_votes FOR ALL USING (auth.uid() = user_id);
...
CREATE POLICY "Admins can manage appearance settings" ON appearance_settings FOR ALL USING (
  EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND is_admin = true)
);
